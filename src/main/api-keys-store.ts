...
export class ApiKeyStore {
  ...
  /** 캐시 hit 시 즉시 반환, miss 시 디스크에서 복호화·추출 후 캐시. 실패는 빈 객체로 안전 fallback. */
  read(): Record<string, string> {
    ...
    try {
      ...
      const fresh: Record<string, string> = (parsed && typeof parsed === 'object')
        ? pickKnownKeys(parsed as Record<string, unknown>)
        : Object.create(null);
      this.cache = fresh;
      return fresh;
    } catch {
      // 파일 미존재/복호화 실패/손상 JSON — 모두 빈 키셋으로 안전 fallback.
      const empty: Record<string, string> = Object.create(null);
      this.cache = empty;
      return empty;
    }
  ...
  save(provider: string, key: string): void {
    ...
    const keys = { ...this.read(), [provider]: key };
    this.write(keys);
  }
