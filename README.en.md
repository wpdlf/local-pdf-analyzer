...
- **Automatic session save & restore (new in v0.18.27)** — Reopen an analyzed PDF and its summary, Q&A conversation, and search index are restored as-is. For identical content there is no re-summarize / re-embed — eliminating cloud embedding token re-billing in particular. Continue from the recent-documents list, or turn it off / clear all in Settings
...
- Previously analyzed PDFs appear in the **Recent Documents** list below the upload area, and reopening the same PDF **automatically restores** its summary, Q&A, and search index (no re-summarize / re-embed, v0.18.27)
...
- **Unit test coverage** — **762 regression tests across 39 files** spanning RAG / citation / Q&A core paths and the main process (renderer·shared 443 + main 319 — started from 463 renderer core-path tests, +13 in v0.17.x / +227 cumulative across v0.18.x). The R32 round in v0.18.19 patch added +30 (P1/P2/P3 cumulative); R33 4-agent QA then surfaced regressions in R32-introduced code which R34 fixed, adding another +30 — R34 P1 +4 (safe-markdown MarkdownErrorBoundary reset guards) and R34 P2 +26 (settings-keys drift guards 6 / enrich-doc Vision partial-failure contract 11 / preload contextBridge shape snapshot 9). v0.18.23 R37 P6 added +49 — use-summarize `stripConversationalText` multilingual filler removal 27 / pdf-parser image-cap real verification (fixed a vacuous pass via OPS·OffscreenCanvas mocks) / `ollama-pull-progress.ts` pull-output parsing extraction 15 / use-qa `buildRagIndex` defensive branches 7. The coverage gate was raised 38/33/41/39 → 44/40/44/46 and is now enforced by a CI `coverage` job. v0.18.24 R38 stood up a **behavior test suite for the previously near-untested main process** (P1~P5, 11 files / 283 tests) — IPC handlers·validators, OllamaManager network·process lifecycle, the API key store, and the ai-service body, all under an electron-mocking harness. v0.18.25 added 6 regression tests for Q&A TF keyword counting with word-boundary matching (dropping the substring false-positive where `ai` matched inside `said`/`rain`) and the verify embedding dimension-mismatch fail-safe guard. v0.18.26 R39/R40 added 5 (`ai:check-available` SSRF store-read guard + fallback). v0.18.27 **session persistence +30** — session-store I/O·LRU 21 / VectorStore serialize round-trip 3 / content hash 4 / restore·autosave flow 10 (in-memory fs mock + electron mock) / file:open-path security guard + store restore fields. v0.19.0 R41 +5 — guards for disk-corruption/race regressions a 4-agent QA found in the new session-persistence code (unlink stale index.bin on blob=null re-save / meta-field normalization). v0.19.1 R42 +3 — manifest per-entry normalization (blocks the path where a corrupted non-string lastAccessed·non-finite byteSize crashed the recent list/stats or disabled the LRU cap).
- **Build integrity (cumulatively hardened across v0.18.8 ~ v0.18.19 patch)** — Each release auto-publishes installer SHA-256 hashes and a Sigstore build provenance attestation. CI workflows pin every third-party action by full SHA, use `npm ci`, and keep the lockfile in sync to guarantee reproducible builds. v0.18.9 adds `timeout-minutes` on every job, an Ubuntu/Windows OS matrix on the test job, and a mandatory `npx tsc --noEmit` gate on both PR and release pipelines so strict flags like `noUncheckedIndexedAccess` cannot silently regress. v0.18.10 pins the Windows runner from `windows-latest` to `windows-2025` ahead of the June 2026 migration. v0.18.11 bumps `actions/checkout` and `actions/setup-node` to their Node.js 24-compatible majors (v6), adds an advisory `npm audit --audit-level=high` step, and declares an `engines` field in `package.json` (node ≥ 20.11, npm ≥ 10). v0.18.13 introduces `asarUnpack: ["**/cmaps/**"]` (so pdfjs CMap files survive asar packing) plus 9 R29 P1 hardening fixes. v0.18.15 adds Ollama `keep_alive: '30m'` + renderer `manualChunks` (main chunk 808→304 KB, -62%) + Vision provider-aware concurrency (Ollama 3 / cloud 8) — first performance-track round. v0.18.16 introduces PdfViewer page virtualization (IntersectionObserver-driven lazy render, ~95% fewer canvases for a 100-page citation click) — second performance-track round. v0.18.17 lands six R30 full-QA P1 fixes (Promise.race timer leak, PdfViewer viewport race, lockfile drift, empty image-name guard, targetPage polling stuck, workflow node-version drift). v0.18.18 bundles six R30 P2 + R29 QA P2 small-fixes (Vision in-flight abort, setNotice auto-dismiss, vitest coverage, LOCALHOST_HOSTS dedup, Bearer regex `~`, shell:open-external length cap). v0.18.19 patch lands the R32 4-agent parallel QA result: 5 P2 + 8 P3 + 12 P4 + 8 P5 — the same v0.18.19 release assets were overwritten three times with the cumulative build (Q&A cross-session token contamination / prompt injection via summary+assistant / PDF parse error banner path leak / OCR cloud abort un-propagated / CI audit step `if: always()` + Vision stale enrichment / theme localStorage drift / MarkdownErrorBoundary reset / OCR memory cap / MAX_LINE_SIZE silent skip / lockfile `packages[""]` check / audit JSON single-spawn consolidation / PowerShell quote helper + 9-case unit test + handleSummarize docId guard / appendQaStream isQaGenerating gate / enrichedPageTexts version counter / clientRef ownership / taskkill SIGKILL fallback / will-redirect tightened / generate placeholder controller / req.setTimeout settled / i18n hasOwnProperty + missing-key fallback / ResizeHandle Home/End ARIA alignment / safe-markdown headings·blockquote citations / vitest pool 'forks' + Vitest 4 migration / ai-client.test timer margin / release.yml fail-fast: false / postbuild cmap smoke check). A follow-up R33 4-agent QA then detected four regressions inside R32-introduced code, which R34 P1 cleaned up (MarkdownErrorBoundary now compares `fallbackText` string instead of `children` element identity / `generate()` placeholder leak wrapped in try/catch with identity-based cleanup / will-redirect's file:// comparison switched to `pathToFileURL().href` standard URL / CI audit step gated by `[ -d node_modules ]` plus an `j.error` branch when fired after `npm ci` failure). R34 P2 closed coverage gaps with new pure-helper modules and accompanying P4 hardening (`VALID_SETTINGS_KEYS` consolidated into `src/main/settings-keys.ts` / `enrichDocumentWithImages` extracted to `src/renderer/lib/enrich-doc.ts` / preload contextBridge shape static snapshot test / i18n `hasOwnProperty` AND `=== undefined` / pdf-parser OCR per-page abort listener race tightened / preload `ai.abort` return type aligned with `error?` / postbuild SMOKE_FILES includes `Adobe-CNS1-UCS2.bcmap`). The same v0.18.19 release assets were overwritten **five times**; Critical remains zero across 34 consecutive QA rounds. Subsequently v0.18.23 R37 made the CI coverage gate actually enforced via a dedicated `coverage` job (previously the thresholds applied only on manual `npm run test:coverage`, so coverage regressions passed CI silently) and aligned the test/release workflow Node from 20.11 → 22 (20.11 EoL April 2026). The 4-agent parallel QA found zero Critical/High; Critical stays zero across 37 consecutive rounds. R38~R40 4-agent parallel QA likewise found zero Critical/High (40 consecutive rounds) — v0.18.26 blocked the `ai:check-available` SSRF port-scan oracle (using the settings-store canonical URL instead of the renderer-supplied URL), bumped electron 41.6.1→41.7.1 (Chromium security rollup), and resolved 6 dev-dependency vulnerabilities via `npm audit fix` (npm audit 0). v0.18.27 completed the session-persistence feature through a full PDCA plan-plus→design→do(4 modules)→check(Match Rate 100%) cycle. Subsequently v0.19.0 R41 4-agent parallel QA found, in the new session-persistence code, 1 High + 5 Important disk-session corruption issues (summaryType↔content mismatch / stale index.bin / persist load-merge-save race / restoredSession provider stale snapshot / unvalidated meta / RecentDocuments mountedRef) and fixed them, promoting the feature to a stabilization milestone; v0.19.1 R42 cleaned up 1 Important + 4 Minor (manifest entry robustness via corrupted-entry normalization + UI error convergence: drag-drop banner / recent-list delete feedback). Critical/High stay zero across all of v0.19.x (42 consecutive rounds through R42).
- **Session persistence + RAG index caching (v0.18.27, stabilized in v0.19.0)** — Per-document content hash (SHA-256) keys a split store under `userData/sessions/`: summaries, Q&A, and parsed text as JSON, the embedding index as a Float32 binary blob (atomic tmp→rename). Reopening the same PDF restores by hash match, and when the embedding model·dimension match, the index is deserialized for **zero re-embed / re-summarize calls**. The restore↔auto-build race is blocked by a `sessionRestorePending` gate + `restoredSession` marker. LRU (max 30 docs / 200MB) auto-eviction; cache safely invalidated on content/model/dimension change. Native-dependency-free (no SQLite). `file:open-path` applies the same security guard as `file:open-pdf` (.pdf extension · symlink rejection · 100MB cap) to block arbitrary file reads on recent-list reopen. v0.19.0~v0.19.1 hardened corruption tolerance — a blob-less re-save removes the stale `index.bin` (preventing dimension-mismatch restores and LRU byteSize under-counting), and both renderer-supplied meta and on-disk manifest entries are server-side normalized (length caps + `Number.isFinite` + epoch fallback for non-string `lastAccessed`) so a manifest corrupted by a partial write/external edit can no longer crash the recent list/stats or defeat the 200MB cap
...
| Every Q&A answer regenerates twice after switching the embedding model | Fixed in v0.18.25 — when the model that built the index and the current verification embedding model have different dimensions, RAG search always returns an empty result, so every sentence is misclassified as weak and refine is force-triggered. A fail-safe guard now skips verification and keeps the draft as-is on a dimension mismatch |
| Reopening the same PDF re-summarizes and re-indexes from scratch | v0.18.27 session persistence — identical-content documents auto-restore their summary, Q&A, and search index. With a matching embedding model there is no re-embed. To disable, turn off the "Session Data" toggle in Settings |
| Opened from the recent list but the source PDF viewer doesn't appear | v0.18.27 — documents are identified by content hash, so the summary/Q&A analysis restores even if the original file moved/was deleted, but PDF viewer rendering is disabled (the analysis is still usable). Reopen the original to restore the viewer |
| Saved sessions take up too much disk | v0.18.27 — LRU keeps at most 30 docs / 200MB, auto-evicting the oldest beyond that. Check current usage and "Clear all" in Settings → "Session Data" |
| A restored summary tab's label doesn't match its content | Fixed in v0.19.0 — when a given summary type wasn't saved, the app would show another type under the original label (e.g. full body under the "Keywords" tab). It now labels the fallback with its actual type and avoids autosaving under the wrong key |
| Keeping the app from freezing when session data is corrupted | Hardened in v0.19.0~v0.19.1 — even if a manifest entry is corrupted by a partial write/external edit, the recent-documents list and usage stats no longer crash; corrupted entries are safely ignored and cleaned up (no action needed) |
...
| Testing | Vitest (762 unit tests across 39 files — renderer·shared 443 / main 319) + `tsc --noEmit` strict type check (`noUncheckedIndexedAccess` enabled in v0.18.8; enforced on both PR and release CI in v0.18.9; `vitest.config.mts` + `test/setup.ts` entry point added in v0.18.11; `noFallthroughCasesInSwitch` + `noImplicitOverride` enabled in v0.18.19; `pool: 'forks'` pinned + Vitest 4 migration in v0.18.19 patch). Native-dep-free pure helpers are extracted into separate modules so vitest can import them directly (`settings-keys.ts` / `ps-quote.ts` / `enrich-doc.ts` / `ollama-pull-progress.ts`, v0.18.23) — drift guards + Vision partial-failure contract + pull-progress parsing statically verified. From v0.18.23 the coverage thresholds (44/40/44/46) are enforced by a CI `coverage` job. v0.18.24 R38 stood up a main-process behavior suite (IPC·OllamaManager·API key store·ai-service, 11 files / 283 tests) under an electron-mocking harness. v0.18.27 session persistence added +30 (session-store with in-memory fs mock · VectorStore serialize round-trip · restore/autosave flow · file:open-path security guard). v0.19.0 R41 +5 (use-session summaryType fallback·persist serialization / session-store stale index.bin·meta normalization) / v0.19.1 R42 +3 (loadManifest corrupted-entry normalization·discard regression guards) |
...
└── renderer/             # React UI
    ...
    │   └── __tests__/         # Unit tests (762 across 39 files)
    └── types/
...
| SSRF port-scan oracle (v0.18.26) | If `ai:check-available` trusts the renderer-supplied URL, a compromised renderer can probe arbitrary localhost ports (host/protocol are validated but the port is not). Switched to using the settings-store canonical URL only, blocking per-call port sweeps (same pattern as the embeddings/vision handlers) |
| Session path traversal (v0.18.27) | Session directories are keyed by content hash — `docHash` is enforced via a `/^[a-f0-9]{64}$/` allowlist to block `..` path manipulation. The `file:open-path` recent-list reopen rejects non-`.pdf` extensions / symlinks and caps at 100MB to block arbitrary file reads. Session data contains no API keys |
...
| Inline script injection | `'unsafe-inline'` removed from CSP `script-src` (v0.18.23) — only the single FOUC-prevention init script is whitelisted via sha256 hash; a vitest gate forces hash refresh whenever the script body changes. `style-src` keeps `'unsafe-inline'` due to pervasive Tailwind/React inline styles (with no inline script, style-only exfil is not reachable) |
...
| Render-error path leak | `AppErrorBoundary` rewrites home-directory paths (`C:\Users\...`, forward-slash `C:/Users/...`, `/Users/...`, `/home/...`) to `~` and truncates to 500 chars (v0.18.23 added forward-slash Windows path coverage — blocks the drive-letter remnant from Node/pdf.js-normalized paths) |
...
| RAG chunk multi-page citation loss (v0.18.21 R35) | Multi-page RAG chunks injected range labels `[p.N-M]` into prompts and relied on the LLM to convert them to single `[p.N]` per body citation. `CITATION_REGEX` only recognizes single labels, so smaller local models that violated the conversion rule produced answers with the citation silently dropped. Changed `formatPageLabel` to always emit a single `[p.N]` (chunk's first page), enforcing Decision #6 in code (R34 P3 regression catch) |
| Chunk page attribution dragged by overlap tail (v0.18.21 R35) | The RAG chunker included the overlap-tail region in page-label coordinate calculation, so even when the body started at page N+1, a single tail token touching page N caused the chunk to be mis-labeled `[p.N]`. Switched to body-only coordinates, separating retrieval coordinates (overlap-aware) from attribution coordinates (body-only) |
| `appendStream` ghost-token race asymmetry (v0.18.21 patch R36 P1) | The `isQaGenerating` gate that R32 P3 added at the entry of `appendQaStream` was missing on the summary-side `appendStream`. After user Stop → `handleAbort` → `setIsGenerating(false)`, an in-flight IPC for-await yielding a token before the next iteration's gate would reset `cleared=false`, letting the 50ms flush leak ghost text into `summaryStream`. Mirrored the QA-side gate to restore symmetry |
| `setEnrichedPageTexts` lacked reference-equal no-op (v0.18.21 patch R36 P2) | R32 P3's monotonic version counter let fingerprints detect a second Vision pass even when lengths matched, but identical references (especially repeated null calls) still bumped the version every time. If `useRagBuilder`'s fingerprint ever includes the version in the `r` branch, this would trigger false-positive rebuilds. Added an idempotent guard that skips the set entirely when `s.enrichedPageTexts === pages` |
| `splitIntoSentences` falsely verified citation clusters (v0.18.21 patch R36 P2) | After R35's single-label format, clusters like `[p.5][p.6][p.7]` became more common. With `\s+` normalization + terminator split, a citation cluster following a period became its own fragment, passed the 15-char filter, became an embedding target in `verifyAnswerSentences`, and produced weak scores that triggered false-positive refines. Strip `[p.N]` / `[p.N\|quote]` tokens before split so only body-meaning sentences are verified |
| IPv6 loopback `http://[::1]` guard mismatch (v0.18.21 patch R36 P3) | The WHATWG URL parser returns `http://[::1]`'s hostname as `[::1]` (brackets included), but `LOCALHOST_HOSTS` only contained `::1` (no brackets), so IPv6 loopback was silently blocked against intent. Discovered while writing Top5 #1 (validateOllamaUrl unit tests). Fixed `isLocalhostHost(hostname)` to normalize wrapping `[ ]` before comparison, and unified the four call sites (ai-service.validateOllamaUrl, index.ts settings:set / ai:generate / ai:check-available) from direct `LOCALHOST_HOSTS.includes()` calls to the single `isLocalhostHost()` helper — also closing the drift risk that those four sites had inherited |
| R36 P4 cosmetic/documentation pass (v0.18.22) | Three runtime-invariant cleanups: (1) pdf-parser `extractPageImages` 5s timeout's pdfjs `AbortSignal` limitation documented in a comment — `Promise.race` only short-circuits result selection while pdfjs's internal op parsing keeps running; no mitigation possible until pdfjs upstream supports abort; (2) Added `eslint-disable-next-line react-hooks/exhaustive-deps` to the `use-summarize` cleanup useEffect's intentionally empty deps — future addition of reactive external state would otherwise risk a stale-closure regression; (3) Moved `noticeDismissTimer` / `NOTICE_DISMISS_MS` module variables next to other debounce timers — visual asymmetry only, HMR dispose handler's closure behavior unchanged |
| isLocalhostHost RFC 3986 strict policy (v0.18.22 M1) | Discovered in the 24th audit round right after R36 P3. `isLocalhostHost('[localhost]')` / `isLocalhostHost('[127.0.0.1]')` returned `true` — but per RFC 3986, `[ ]` is reserved for IPv6 IP-literals only. The WHATWG URL parser throws on such inputs, so they were unreachable from the IPC boundary, but the latent gap would have allowed non-canonical bypasses if a future raw-socket caller were added. Strict guard added: when brackets are present but the inner content lacks `:`, return `false` immediately — fail-fast on RFC 3986-violating inputs. Replaced the `[localhost] === true` assertion in constants.test.ts and added a new strict-policy regression guard (5 assertions) |
| stripCitations single source of truth (v0.18.22 C-L1) | `use-qa.splitIntoSentences` (R36 P2-b)'s inline strip regex `[^\]]*?` (non-greedy) was asymmetric with `citation.CITATION_REGEX`'s `[^\]]*` (greedy). They produce identical results on well-formed inputs (since `]` terminates the match anyway), but future changes to the quote-escape policy could introduce silent skew. Added `stripCitations(text)` helper to `citation.ts` as the single source. The helper clones `CITATION_REGEX` per call to avoid g-flag lastIndex accumulation (same dispose pattern as `parseCitations`). `use-qa` now calls the helper; 9 regression guards added |
