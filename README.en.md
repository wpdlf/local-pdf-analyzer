...
- **Unit test coverage** — **463 regression tests** for RAG / citation / Q&A core paths (+13 in v0.17.x, +227 cumulative across v0.18.x, 29 test files). The R32 round in v0.18.19 patch added +30 (P1/P2/P3 cumulative); R33 4-agent QA then surfaced regressions in R32-introduced code which R34 fixed, adding another +30 — R34 P1 +4 (safe-markdown MarkdownErrorBoundary reset guards) and R34 P2 +26 (settings-keys drift guards 6 / enrich-doc Vision partial-failure contract 11 / preload contextBridge shape snapshot 9). v0.18.23 R37 P6 added +49 — use-summarize `stripConversationalText` multilingual filler removal 27 / pdf-parser image-cap real verification (fixed a vacuous pass via OPS·OffscreenCanvas mocks) / `ollama-pull-progress.ts` pull-output parsing extraction 15 / use-qa `buildRagIndex` defensive branches 7. The coverage gate was raised 38/33/41/39 → 44/40/44/46 and is now enforced by a CI `coverage` job
...
| Testing | Vitest (463 unit tests across 29 files) + `tsc --noEmit` strict type check (`noUncheckedIndexedAccess` enabled in v0.18.8; enforced on both PR and release CI in v0.18.9; `vitest.config.mts` + `test/setup.ts` entry point added in v0.18.11; `noFallthroughCasesInSwitch` + `noImplicitOverride` enabled in v0.18.19; `pool: 'forks'` pinned + Vitest 4 migration in v0.18.19 patch). Native-dep-free pure helpers are extracted into separate modules so vitest can import them directly (`settings-keys.ts` / `ps-quote.ts` / `enrich-doc.ts` / `ollama-pull-progress.ts`, v0.18.23) — drift guards + Vision partial-failure contract + pull-progress parsing statically verified. From v0.18.23 the coverage thresholds (44/40/44/46) are enforced by a CI `coverage` job |
...
└── renderer/             # React UI
    ...
    │   └── __tests__/         # Unit tests (463 across 29 files)
    └── types/
