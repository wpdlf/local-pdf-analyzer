  ...
  test: {
    ...
    coverage: {
      ...
      exclude: [
        ...
        // % 게이트 제외. E2E 스모크(app-launch/upload-errors)가 기동·배선 회귀를 가드한다.
        // 단위 행위 테스트 도입 시 본 라인 제거(절차대로).
        //
        // QA24(D-I4) 재확인: 438 → 779줄로 자랐고 단위 커버리지는 여전히 **0%** 다. 분모에
        // 넣어도 게이트로 가드되는 게 없고(0% 하한은 무의미) 전역 지표만 끌어내려 나머지의
        // 회귀 감도를 떨어뜨리므로 제외를 유지한다. index.ts 와 달리 여긴 회귀 넷이 아직 없다 —
        // 단위 테스트를 만드는 것이 선행 과제이고, 그 전에 분모만 넣는 것은 숫자 조작에 가깝다.
      ],
      ...
      // QA24(D-I4): 제외 근거가 낡았다. `src/main/index.ts` 는 근거 측정 시점(R38, v0.18.23)의
      //   908줄에서 **1434줄**로, `src/renderer/App.tsx` 는 438 → **779줄**로 자랐다. 둘이 합쳐
      //   2,213줄 ≈ 비테스트 src 의 10% 이고 저장소 내 최대·2위 파일이다. 그 사이 index.ts 에는
      //   창 수명주기·종료 flush handshake·updater 배선이 들어왔는데, 이는 **QA16·17·18·19·23 이
      //   연속으로 결함을 낸 바로 그 영역**이다 — 즉 결함 밀도가 가장 높은 코드가 % 게이트 밖에
      //   영구 상주하고 있었다.
      //
      //   그렇다고 전역 분모에 그냥 넣으면 전 지표가 게이트 아래로 떨어지고(실측 80.94/73.21/
      //   79.29/84.09), 전역 임계를 그만큼 낮추면 **나머지 90% 의 회귀 감도**가 같이 떨어진다.
      //   그래서 glob 임계로 분리한다 — Vitest 는 glob 에 매칭된 파일을 전역 집계에서 제외하므로,
      //   전역 게이트는 종전 강도를 유지하면서 두 파일은 자기 하한으로 가드된다.
      //
      //   개별 실측(2026-08-12): index.ts 53.47/35.76/46.78/56.04 (d1cd6e6 의 합성층 회귀 넷으로
      //   R38 당시 39% 에서 상승) / App.tsx 0/0/0/0 (앱 셸 — 단위 테스트 없음, E2E 가 가드).
      //   각각 −5pp 마진. App.tsx 는 0 이라 하한이 무의미하므로 **분모 편입만** 하고(신규 코드가
      //   조용히 사라지지 않게) 게이트는 걸지 않는다. 단위 행위 테스트가 생기면 하한을 올린다.
      // QA24(D-I4): `src/main/index.ts` 를 **분모에 편입**한다(exclude 라인 제거 — 절차대로).
      //   제외 근거가 낡았다: 근거 측정 시점(R38, v0.18.23)의 908줄 / 전체 39% 에서 지금은
      //   **1434줄 / 53.47·35.76·46.78·56.04** 다. 그 사이 창 수명주기·종료 flush handshake·
      //   updater 배선이 들어왔고, 이는 **QA16·17·18·19·23 이 연속으로 결함을 낸 바로 그 영역**
      //   이다 — 결함 밀도가 가장 높은 코드가 % 게이트 밖에 상주하고 있었다. d1cd6e6 의 합성층
      //   회귀 넷으로 커버리지가 39% → 56% 로 올라 이제 편입해도 드래그가 크지 않다.
      //
      //   편입 후 실측(2026-08-12): Stmts 84.2 / Branch 76.43 / Funcs 83.65 / Lines 87.34.
      //   현행 게이트(81/74/81/84)를 그대로 통과하지만, -5pp 정책대로 재정렬한다. 숫자만 보면
      //   하향이나 **분모가 1,434줄 늘었으므로 절대 기준은 오히려 강해진다**
      //   (0.79×7713 = 6093줄 요구 vs 종전 0.81×6279 = 5086줄).
      //   동시에 QA24(D-M3)의 "게이트가 5.6~6.2pp 뒤처져 -5pp 정책 위반" 도 해소된다.
      thresholds: {
        statements: 79,
        branches: 71,
        functions: 78,
