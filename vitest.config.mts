  ...
  test: {
    ...
    coverage: {
      ...
      exclude: [
        ...
        // QA25(B-High): App.tsx 를 **분모에 편입**한다(exclude 라인 제거 — 절차대로).
        // 제외 근거였던 "회귀 넷이 아직 없다" 가 해소됐다: App.update-banner.test.tsx 가
        // 자동 업데이트 배너 배선을 실제 렌더로 가드한다(이전 유일 가드는 소스 정규식이라
        // 구독을 죽여도·렌더 게이트를 좁혀도 통과했다).
        //
        // QA24(D-I4) 주석은 App.tsx 를 "0%" 로 보고 glob 임계 분리가 필요하다고 계획했으나,
        // 실제로 편입해 보니 **54.01/34.22/70.51/53.2** 다 — 앱 셸을 실제로 렌더하는 테스트가
        // 하나 생기면 배선·레이아웃이 함께 지나가기 때문이다. 전역은 84.38→83.57 로 0.8pp
        // 내려갈 뿐 게이트(79/71/78/82)를 여유 있게 통과하므로 glob 분리는 불필요했다.
      ],
