...
- **교차 문서 요약 / 비교** — 컬렉션 모드에서 **통합 요약**·**비교 분석** 버튼으로 선택 문서들을 종합합니다(각 문서의 기존 요약을 재사용 — 재요약 없음). 결과는 Q&A 스레드에 문서별 출처로 표시됩니다.
...
**품질 보증**
- 단위 테스트 958건 + CI 품질 게이트, 릴리즈마다 4-에이전트 병렬 QA 수행
...
| 테스트 | Vitest 단위 테스트 958건/53파일 (renderer·shared 575 + main 383) + `tsc --noEmit` 타입 체크 + CI 커버리지 게이트 (57/51/58/59) |
...
- **단위 테스트 958건 / 53파일** — renderer·shared 575 + main 383. 메인 프로세스는 electron 모킹 하니스로 IPC 핸들러·OllamaManager·API 키 저장소·ai-service까지 행위 테스트
