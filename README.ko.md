...
**품질 보증**
- 단위 테스트 942건 + CI 품질 게이트, 릴리즈마다 4-에이전트 병렬 QA 수행
...
| 테스트 | Vitest 단위 테스트 942건/53파일 (renderer·shared 564 + main 378) + `tsc --noEmit` 타입 체크 + CI 커버리지 게이트 (57/51/58/59) |
...
- **단위 테스트 942건 / 53파일** — renderer·shared 564 + main 378. 메인 프로세스는 electron 모킹 하니스로 IPC 핸들러·OllamaManager·API 키 저장소·ai-service까지 행위 테스트
