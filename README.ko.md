...
**품질 보증**
- 단위 테스트 2099건 + Playwright E2E + CI 품질 게이트, 릴리즈마다 4-에이전트 병렬 QA 수행
...
| 테스트 | Vitest 단위 테스트 2099건/108파일 (renderer·shared 1362 + main 737) + Playwright E2E (CI-결정적 10건) + `tsc --noEmit` 타입 체크 + CI 커버리지 게이트 (80/72/80/83) |
...
- **단위 테스트 2099건 / 108파일** — renderer·shared 1362 + main 737. 메인 프로세스는 electron 모킹 하니스로 IPC 핸들러·OllamaManager·API 키 저장소·ai-service·전체 문서 검색까지 행위 테스트, 렌더러/preload 레이어(컴포넌트 17종 전수 + 앱 셸 자체 + use-summarize/use-session/pdf-parser/safe-markdown 등 핵심 라이브러리 + preload 브리지)는 happy-dom 으로 행위 테스트
...
- **CI 게이트** — `tsc --noEmit`(strict, e2e 전용 타입체크 프로젝트 포함), 커버리지 임계(80/72/80/83) 강제, lockfile 버전 동기화 검증, 태그 ↔ `package.json` 버전 일치 검증, 차단성 `npm audit` 게이트 2종(프로덕션 트리 전량 + 실제 배포되는 것의 **전이 의존**까지 lockfile 폐포로 검사), 수식 청크가 초기 번들로 새지 않는지 보는 빌드 시점 검사, Node 22/24 매트릭스 + Windows 유닛 테스트 레그
