...
**품질 보증**
- 단위 테스트 1961건 + Playwright E2E + CI 품질 게이트, 릴리즈마다 4-에이전트 병렬 QA 수행
...
| 테스트 | Vitest 단위 테스트 1961건/106파일 (renderer·shared 1251 + main 710) + Playwright E2E (CI-결정적 10건) + `tsc --noEmit` 타입 체크 + CI 커버리지 게이트 (79/71/78/82) |
...
- **단위 테스트 1961건 / 106파일** — renderer·shared 1251 + main 710. 메인 프로세스는 electron 모킹 하니스로 IPC 핸들러·OllamaManager·API 키 저장소·ai-service·전체 문서 검색까지 행위 테스트, 렌더러/preload 레이어(컴포넌트 17종 전수 + 앱 셸 자체 + use-summarize/use-session/pdf-parser/safe-markdown 등 핵심 라이브러리 + preload 브리지)는 happy-dom 으로 행위 테스트
