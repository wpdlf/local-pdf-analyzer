...
### 5. 페이지 인용 + PDF 뷰어 (v0.17.0 신규)
- 요약과 Q&A 답변의 주요 문장 끝에 자동으로 **`[p.12]` 같은 페이지 인용**이 표시됩니다
...
- **단위 테스트 커버리지** — 핵심 RAG 경로(벡터 스토어·청커·surrogate pair 안전성·CJK 오버랩·citation 파싱) 회귀 방지 테스트 **69건**
- **페이지 인용 자동 생성 + 사이드 PDF 뷰어 (v0.17.0)** — 요약/Q&A 답변에 출처 페이지 `[p.N]` 자동 표시, 클릭 시 우측 패널에서 해당 페이지 즉시 확인. RAG 청크에 page 메타데이터 전파 + LLM 프롬프트 CITATION_RULES(5 언어) 주입 + pdfjs-dist lazy 뷰어 + react-markdown text-block 오버라이드로 구현
