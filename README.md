...
- **단위 테스트 커버리지** — 핵심 RAG/citation/Q&A 경로 회귀 방지 테스트 **322건** (v0.17.x 에서 +13, v0.18.x 누적 +227, 18 test files). v0.18.19 patch 의 R32 라운드에서 +30 (P1/P2/P3 누적), R33 4-에이전트 QA 가 발견한 회귀를 R34 가 정리하며 추가 +30 — P1 4 (safe-markdown MarkdownErrorBoundary reset 가드) + P2 26 (settings-keys drift 가드 6 / enrich-doc Vision partial-failure 계약 11 / preload contextBridge shape snapshot 9)
...
| 테스트 | Vitest (322개 단위 테스트, 18 파일) + `tsc --noEmit` strict 타입 체크 (`noUncheckedIndexedAccess` 활성, v0.18.8; PR/release CI 양쪽에서 강제, v0.18.9; `vitest.config.mts` + `test/setup.ts` 진입점 도입, v0.18.11; `noFallthroughCasesInSwitch` + `noImplicitOverride` 활성, v0.18.19; `pool: 'forks'` 명시 + Vitest 4 마이그레이션, v0.18.19 patch). R34 P2 에서 main 측 native deps 없는 pure helper 들을 별도 모듈로 분리하여 vitest 가 직접 테스트 가능 (`settings-keys.ts` / `ps-quote.ts` / `enrich-doc.ts`) — drift 가드 + Vision partial-failure 계약 정적 검증 |
