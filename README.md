...
**Quality assurance**
- 1362 unit tests + Playwright E2E + CI quality gates, plus a 4-agent parallel QA round on every release
...
| Testing | Vitest, 1362 unit tests / 82 files (renderer·shared 906 + main 456) + Playwright E2E (9 CI-deterministic tests) + `tsc --noEmit` type check + CI coverage gates (77/69/79/81) |
...
└── renderer/             # React UI
    ...
    │   └── __tests__/         # Unit tests (1362, 82 files)
    └── types/
