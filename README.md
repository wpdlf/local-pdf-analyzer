...
| 테스트 | Vitest (292개 단위 테스트) + `tsc --noEmit` strict 타입 체크 (`noUncheckedIndexedAccess` 활성, v0.18.8; PR/release CI 양쪽에서 강제, v0.18.9; `vitest.config.mts` + `test/setup.ts` 진입점 도입, v0.18.11; `noFallthroughCasesInSwitch` + `noImplicitOverride` 활성, v0.18.19; `pool: 'forks'` 명시 + Vitest 4 마이그레이션, v0.18.19 patch) |
...
| postbuild cmap 부분 복사 silent (v0.18.19 patch) | `cpSync` 가 ENOSPC / 중간 실패로 부분 복사된 채 빠져나오면 NSIS 가 깨진 cmap 세트로 패키징되어 사용자가 설치 후에야 CJK 글리프 깨짐을 발견하던 결함. 대표 cmap 3개 (`Adobe-Japan1-UCS2.bcmap`, `Adobe-Korea1-UCS2.bcmap`, `Adobe-GB1-UCS2.bcmap`) 존재 smoke check |
