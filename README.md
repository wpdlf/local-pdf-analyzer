...
**개인 PC에서 직접 돌아가는 로컬 AI 기반 PDF 요약 도구입니다.**
...
- **텍스트 + 이미지 통합 분석** — 강의자료, 논문, 보고서 등 어떤 PDF든 텍스트는 물론 차트, 다이어그램, 표 등 삽입 이미지까지 Vision AI로 분석합니다
...
- **깔끔한 요약 결과** — AI가 생성하는 불필요한 인사말, 감상평, 대화형 멘트 없이 요약 본문만 출력
...
| 요약에 "잘 정리해주셨네요" 같은 문구가 나옴 | v0.8.3 이상으로 업데이트하면 자동으로 해결됩니다 |
...
| 유형 | 프롬프트 핵심 지시 |
|------|-------------------|
| `full` | 핵심 개념, 주요 내용, 수식/공식, 예제, 시험 포인트 5개 항목 구조 |
| `chapter` | 해당 섹션의 개념/정의, 수식, 예제, 3~5개 핵심 포인트 |
| `keywords` | 키워드/설명/중요도 마크다운 테이블 (10~30개) |

**금지 사항** (전 유형 공통): 인사말, 칭찬, 감상평, 대화형 멘트를 출력하지 않도록 명시적으로 지시합니다. 이를 통해 "잘 정리해주셨네요", "궁금한 점이 있으면 말씀해주세요" 같은 불필요한 텍스트가 요약 결과에 포함되지 않습니다.
