...
**Quality assurance**
- 1272 unit tests + Playwright E2E + CI quality gates, plus a 4-agent parallel QA round on every release
...
| Testing | Vitest, 1272 unit tests / 80 files (renderer·shared 844 + main 428) + Playwright E2E (8 CI-deterministic tests) + `tsc --noEmit` type check + CI coverage gates (77/69/79/81) |
...
└── renderer/             # React UI
    ...
    │   └── __tests__/         # Unit tests (1272, 80 files)
    └── types/
