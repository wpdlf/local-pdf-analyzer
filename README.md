...
**Quality assurance**
- 1432 unit tests + Playwright E2E + CI quality gates, plus a 4-agent parallel QA round on every release
...
| Testing | Vitest, 1432 unit tests / 83 files (renderer·shared 944 + main 488) + Playwright E2E (9 CI-deterministic tests) + `tsc --noEmit` type check + CI coverage gates (80/72/80/83) |
...
└── renderer/             # React UI
    ...
    │   └── __tests__/         # Unit tests (1432, 83 files)
    └── types/
