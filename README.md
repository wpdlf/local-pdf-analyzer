...
**Quality assurance**
- 1294 unit tests + Playwright E2E + CI quality gates, plus a 4-agent parallel QA round on every release
...
| Testing | Vitest, 1294 unit tests / 80 files (renderer·shared 858 + main 436) + Playwright E2E (8 CI-deterministic tests) + `tsc --noEmit` type check + CI coverage gates (77/69/79/81) |
...
└── renderer/             # React UI
    ...
    │   └── __tests__/         # Unit tests (1294, 80 files)
    └── types/
