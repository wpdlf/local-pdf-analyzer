...
**Quality assurance**
- 1278 unit tests + Playwright E2E + CI quality gates, plus a 4-agent parallel QA round on every release
...
| Testing | Vitest, 1278 unit tests / 80 files (renderer·shared 848 + main 430) + Playwright E2E (8 CI-deterministic tests) + `tsc --noEmit` type check + CI coverage gates (77/69/79/81) |
...
└── renderer/             # React UI
    ...
    │   └── __tests__/         # Unit tests (1278, 80 files)
    └── types/
...
- **4-agent parallel QA** — a full-codebase QA round on every release; zero Critical findings across 48+ consecutive rounds (detected High/Important issues are fixed before release — e.g. R41 caught a High session-corruption path in v0.19.0; recent rounds surface only Low/cosmetic items — the most recent batch was triaged and shipped as fixes in v0.31.2)
- Detailed improvement/fix history: [docs/HISTORY.md](docs/HISTORY.md) (Korean)
