...
**Quality assurance**
- 1456 unit tests + Playwright E2E + CI quality gates, plus a 4-agent parallel QA round on every release
...
| Testing | Vitest, 1456 unit tests / 85 files (renderer·shared 958 + main 498) + Playwright E2E (9 CI-deterministic tests) + `tsc --noEmit` type check + CI coverage gates (80/72/80/83) |
...
└── renderer/             # React UI
    ...
    │   └── __tests__/         # Unit tests (1456, 85 files)
    └── types/
