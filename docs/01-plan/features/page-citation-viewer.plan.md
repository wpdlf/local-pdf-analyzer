...
| FR-09b | 가로 리사이즈 핸들 | Medium | ⏸ Deferred to v0.17.x (50/50 고정 split 사용, Decision Record 참조) |
...
| ID | 항목 | 결정 | 이유 |
|---|---|---|---|
| DR-01 | FR-09b 가로 리사이즈 핸들 | v0.17.x 후속 (deferred) | Medium 우선순위, 50/50 고정 split 으로 핵심 use case 충족. 리사이즈 핸들 추가 시 ResizeHandle 컴포넌트 + 너비 store 필드 + 드래그 이벤트 (~50 LOC) 필요. v0.17.0 출시 후 사용자 피드백 기반 결정. |
